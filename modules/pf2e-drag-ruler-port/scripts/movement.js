function movementSpeed(token, type) {
  ...
  if (token.actor.type === "character" && type === "land") {
    type = "land-speed";
  }
  if (token.actor.type !== "vehicle") {
    let findSpeed =
      token?.actor?.system?.attributes?.speed?.otherSpeeds?.find(
        (e) => e.type == type,
      ) ?? 0;
    if (findSpeed?.total !== undefined) {
      return {
        speed:
          findSpeed?.total > 0
            ? findSpeed?.total
            : parseFloat(findSpeed?.breakdown?.match(/\d+(\.\d+)?/)[0]),
        type: type,
      }; //if a matching speed if found returns it.
    } else if (
      token.actor.system.attributes?.speed?.total !== 0 &&
      isNaN(token.actor.system.attributes?.speed?.total) == false
    ) {
      ...
      return {
        speed: parseFloat(token.actor.system.attributes?.speed?.total) ?? 0,
        type: token.actor.system.attributes?.speed?.type ?? "default",
      };
    } else {
      return {
        speed: Math.max(
          [
            parseFloat(
              token.actor.system.attributes?.speed?.breakdown?.match(
                /\d+(\.\d+)?/,
              )[0],
            ),
            parseFloat(
              token.actor.system.attributes?.speed?.otherSpeeds[0]?.total,
            ),
            0,
          ].filter(Number),
        ),
        type: "special",
      }; //pulls out the speed for the value string in the event that the total was 0. In the event that both the total and value for the land speed are 0, falls back on the first other speed total, should one not exist speed will be 0.
    }
    //handles speeds for vehicles because they're different.
  } else if (token.actor.type === "vehicle") {
    return {
      speed: parseFloat(token.actor.system.details?.speed),
      type: "default",
    };
  }
}
...
function getMovementType(token) {
  ...
  //     if(tokenElevation > 0) {movementType = 'fly'}; //if elevated fly
  ...
  if (token.actor.flags.pf2e?.movement?.burrowing === true) {
    movementType = "burrow";
  } //switches to burrowing if the burrow effect is applied to the actor.
  if (token.actor.flags.pf2e?.movement?.climbing === true) {
    movementType = "climb";
  } //switches to climbing if the climb effect is applied to the actor.
  if (token.actor.flags.pf2e?.movement?.swimming === true) {
    movementType = "swim";
  } //switches to swimming if the swim effect is applied to the actor.
  if (token.actor.flags.pf2e?.movement?.flying === true) {
    movementType = "fly";
  } //switches to flying if the fly effect is applied to the actor.
  if (token.actor.flags.pf2e?.movement?.walking === true) {
    movementType = "land";
  }
  return movementType;
}
